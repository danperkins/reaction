...
import StaticExerciseCatalog from '../../components/ExerciseCatalog/StaticCatalog';
import StaticWorkoutHistory from '../../components/WorkoutHistory/StaticWorkoutHistory';
...
describe("ExerciseFinder component", () => {
    ...
    it("can make an http request", () => {
        let p = axios.get('http://foo.com').then((res) => {
            chai.expect(res, 'has value').to.not.be.undefined;
        });

        window.setTimeout(() => {
            reqs[0].respond(200, { 'Content-Type': 'application/json' }, JSON.stringify({data: 'testdata'}));
        }, 0);
        return p;
    });

    it("should REALLY render ExerciseFilters and ExerciseCatalog", () => {
        let exerciseFinder = enzyme.shallow(<ExerciseFinder addNewWorkout={() => {}} catalog={[]} />);
    ...
    it("should clear workouts", () => {
        ...
        let exerciseFinder = enzyme.mount(<ExerciseFinder addNewWorkout={() => {}} catalog={[]} />);

        let workout: IWorkout = { exercises: ['test Exercise', 'another Exercise'] };
        ...
        chai.expect(exerciseFinder.state().currentWorkout, "workout cleared").to.be.deep.equal({ exercises: [] });
    });

    it('should add a workout to history', () => {
        let callback: Sinon.SinonSpy;
        let testExercises = ['test Exercise'];
        let workout: IWorkout = { exercises: testExercises };
      ...
      it('addExerciseToCurrentWorkout callback should update the state', () => {
          let exerciseFinder = enzyme.shallow(<ExerciseFinder addNewWorkout={() => {}} catalog={StaticExerciseCatalog} />);
          let inst = exerciseFinder.instance() as ExerciseFinder;
      ...
      it('should filter exercises based on Search', () => {
          let exerciseFinder = enzyme.mount(<ExerciseFinder addNewWorkout={() => {}} catalog={StaticExerciseCatalog} />);
          let inst = exerciseFinder.instance() as ExerciseFinder;
          let allItems = inst.getFilteredItems();
          chai.expect(allItems.length, 'all items should be visible').to.equal(StaticExerciseCatalog.length);
